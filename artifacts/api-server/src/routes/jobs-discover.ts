...
router.post("/jobs/discover", authMiddleware, async (req, res) => {
  ...
  // ── Quota enforcement — identical rules to Find Jobs ─────────────────────
  //   Pro users   → 10 searches per CV per day (requires CV selection)
  //   Non-Pro     → deducts 1 from shared job_rec_credits pool
  ...
  if (!effectiveSkipCache) {
    ...
    if (cached) {
      ...
      if (cachedJobs.length > 0) {
        ...
        if (!isPro) await spendJobRecCredit(userId);
        return res.json({
